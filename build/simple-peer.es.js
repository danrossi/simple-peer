...
class SDPUtils {
   ...
   * @param {*} extensions 
   */
  static addExtensions(media, extensions = []) {

    let highestValue = media.ext.reduce((acc, value) => {
      return (acc = acc > value.value ? acc : value.value);
    }, 0) + 1;

    const ext = extensions.map(extension => { return { value: highestValue++, uri: extension } });
    media.ext = [...media.ext, ...ext];
  }
      ...
      sdp.media.map(media => {
        switch (media.type) {
          ...
          case "audio":
            ...
            if (hasExtensions) this.addExtensions(media, extensions);
            break;
        }
